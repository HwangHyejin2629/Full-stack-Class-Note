...
public class OrderService {
	...
	public List<ProductDTO> save(OrderDTO dto){
		...
		//상품 존재 여부 확인
		//select *from product;
		Optional<ProductEntity> option=productRepository.findById(dto.getProductId()); //반환형이 oprional
		...
		if(option.isPresent()) {
			...
			//잘못된 또는 부적절한 인수가 메서드에 전달됐을때 발생하는 예외
			throw new IllegalArgumentException("상품을 찾을 수 없습니다");
		}
		
		//재고확인
		if(productEntity.getProductStock()<dto.getProductCount()) {
			throw new RuntimeException("재고가 부족합니다. 현재 재고는"+productEntity.getProductStock());
		} // 예외 발생되면 멈춘다. 다음으로 넘어가지 않음
